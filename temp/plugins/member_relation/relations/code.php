<?php
  ...
  if ( !defined('CHECK_INCLUDED') ){
	...
	$mymemberrelationdetail = new MemberRelationDetail($myconnection);
	$mymemberrelationdetail->connection = $myconnection;
...
//for pagination
	...
	if(isset($_GET["lstuser"])){
        ...
        $data_bylimit = $mymemberrelationdetail->get_list_array_bylimit($search_string, $user_id,$Mypagination->start_record,$Mypagination->max_records);
        
        if ( $data_bylimit == false ){
            $mesg = "No records found";
        }else{
            $count_data_bylimit=count($data_bylimit);
            $Mypagination->total_records = $mymemberrelationdetail->total_records;
