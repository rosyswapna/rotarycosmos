...
function get_id(){
    $strSQL = "SELECT id,name FROM memberrelations WHERE name = '".$this->name."'";
...
function get_detail(){
    $strSQL = "SELECT id,name FROM memberrelations WHERE id = '".$this->id."'";
    $rsRES = mysql_query($strSQL,$this->connection) or die ( mysql_error() . $strSQL );
    if ( mysql_num_rows($rsRES) > 0 ){
        $this->id = mysql_result($rsRES,0,'id');
        $this->name = mysql_result($rsRES,0,'name');
...
function update(){
    ...
    $strSQL = "INSERT INTO memberrelations (name) VALUES ('";
    ...
    elseif($this->id > 0 ) {
    $strSQL = "UPDATE memberrelations SET name = '".addslashes(trim($this->name))."'";
...
function get_array(){
        ...
        $strSQL = "SELECT id,name FROM memberrelations ORDER BY name";
        $rsRES = mysql_query($strSQL,$this->connection) or die(mysql_error(). $strSQL );
        if ( mysql_num_rows($rsRES) > 0 ){
        while ( list ($id,$name) = mysql_fetch_row($rsRES) ){
          $memberrelations[$id]["name"] = $name;
        }
        return $memberrelations;
        }
        else{
        $this->error_number = 4;
        $this->error_description="Can't list memberrelations";
        return false;
        }
}
...
function get_list_array(){
        $memberrelations = array();$i=0;
        $strSQL = "SELECT id,name FROM memberrelations ORDER BY name";
        $rsRES = mysql_query($strSQL,$this->connection) or die(mysql_error(). $strSQL );
        if ( mysql_num_rows($rsRES) > 0 ){
        while ( list ($id,$name) = mysql_fetch_row($rsRES) ){
              $memberrelations[$i]["id"] = $id;
              $memberrelations[$i]["name"] = $name;
              $i++;
        }
        return $memberrelations;
        }
...
function get_list_array_bylimit($name=-1,$start_record = 0,$max_records = 25){
        ...
        $strSQL = "SELECT id,name FROM memberrelations WHERE 1 ";
        if ( $name != "" && $name != -1 ) {
